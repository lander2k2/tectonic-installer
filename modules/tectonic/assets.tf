...
resource "template_dir" "tectonic" {
  ...
  vars {
    ...
    ingress_tls_cert = "${base64encode(var.existing_certs["ingress_crt_path"] == "/dev/null" ? join(" ", tls_locally_signed_cert.ingress.*.cert_pem) : file(var.existing_certs["ingress_crt_path"]))}"
    ingress_tls_key  = "${base64encode(var.existing_certs["ingress_crt_path"] == "/dev/null" ? join(" ", tls_private_key.ingress.*.private_key_pem) : file(var.existing_certs["ingress_key_path"]))}"
